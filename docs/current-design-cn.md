...
**当前主线**：ODW 编排 + DDW adapter + DeepSeek 贯穿缓存 + ReasoniX per-agent harness + 动态工作流可视化

## 0. 项目定位与开源基调

动态工作流已经证明了它在智能体编排上的效率：它可以把复杂任务拆成多个阶段，让多个 agent 并行探索、交叉验证、汇总产物，并最终形成更高质量的结果。现在动态工作流正在走向开放，技术理应继续流动，让更多人能够使用、理解和改进这种范式。

DDW 的第一个定位，是把动态工作流做得更好，也让更多人用得起动态工作流。很多真实场景里，成本是限制多 agent 工作流普及的关键因素；而 DeepSeek 的高缓存命中机制、便宜的价格和稳定的性能，给了我们一个新的机会：通过 cache-first 的设计，把动态工作流的运行成本显著降下来，同时保留并行智能体编排带来的效率和价值。

我们公开发布 DDW，是希望这套能力能够普惠到更多研究者、开发者、团队和非商业项目。动态工作流能做的不只是几个 demo：代码库审计、政策研究、法律冲突挖掘、多城市深度研究、网页证据提取、复杂报告生成，都只是开始。DDW 的目标，是让这些原本昂贵、复杂、难以观测的 agent workflow，变成可运行、可复用、可观测、可负担的工程系统。

## 1. 一句话定位

DDW 是一个 cache-first 的动态工作流 adapter 层。它把 Open Dynamic Workflows 的 `agent()` 节点映射成可观测、可缓存、可替换后端的 agent run，并把每个 agent 的 token、工具调用、缓存命中、耗时、产物和状态沉淀为统一的工作流执行视图。

当前版本的核心判断是：

```text
ODW / DDW 负责编排、缓存策略、产物协议和可视化。
ReasoniX 负责复杂 agent 的高自治执行环境。
Native DDW agent 负责简单、低成本、强可控的轻量任务。
```
...
flowchart TD
  ...
  Agent --> Adapter["DDW Custom Adapter"]
...
DDW 是 ODW 和具体 agent runtime 之间的产品层，负责：
...
DeepSeek 提供模型推理和 prompt cache usage 字段。DDW 当前读取：
...
DDW 不依赖 agent 自己汇报缓存命中。所有命中指标必须由系统从 API usage 或 ReasoniX transcript 中读取。
...
```text
one ODW agent = one ReasoniX run = one transcript = one DDW session
```
...
Native DDW agent 当前使用内部 JSON 协议：
...
ODW 的 workspace copy / inplace 模式可以和 DDW 的 artifact dir 配合。产品默认应该让 agent 把稳定产物写到 `.cf-dw/runs/<session_id>/artifacts`，避免后续阶段不知道去哪找文件。
...
| 任务类型 | 推荐后端 | 原因 |
|---|---|---|
| 简单摘要 / 标签 / 分类 | Native DDW | 成本低、稳定、可控 |
| 多步分析 / 多工具读取 | ReasoniX | harness 能力强 |
| 代码库审计 / 修改 | ReasoniX | 文件理解、工具策略更强 |
| 大规模 fan-out 的轻任务 | Native DDW | 吞吐高 |
...
8. backend chips：`native-DDW` / `reasonix` 等。
...
3. ODW 的 workspace copy 产物如何和 DDW artifact dir 合并展示。
4. Pro 路由是由 agent 显式请求，还是由 DDW 按 phase complexity 自动决定。
5. Self-Evolve 生成的 skill 应该存为 DDW skill、ReasoniX skill，还是 ODW workflow template。
...
2026-06-10 根据 GitHub Blog《Improving token efficiency in GitHub Agentic Workflows》的经验，DDW 增加了一个新的工程判断：不能只看 total tokens 和 cache hit rate，还要看加权后的 effective tokens。
...
这让 DDW 的成本观测从“命中率”升级为“命中率 + 输出膨胀 + 循环风险 + artifact 产出”的组合视角。
