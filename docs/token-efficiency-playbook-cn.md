# CFDW Token Efficiency Playbook
...
这份 playbook 把 GitHub agentic workflow 的 token efficiency 经验转成 CFDW 的工程规则。目标不是单纯少花 token，而是在动态工作流仍然能并行探索、交叉验证、产出 artifact 的前提下，让成本、缓存命中、输出膨胀和 runaway loop 都可观测、可审计、可优化。

## 1. 统一 usage ledger

GitHub 的核心经验之一是：先把每一次模型调用的用量标准化记录下来，再谈优化。CFDW 当前已经把 Native worker 和 ReasoniX transcript 都归一到每个 run 的 `usage.jsonl`：
...
只看 `total_tokens` 不够，因为 cache-read token、miss input token 和 output token 的成本压力不同。GitHub 文章使用了一个加权口径，CFDW 当前采用同一类有效 token 指标：
...
如果某个 provider 没有 hit/miss 字段，则 CFDW 回退为：
...
GitHub 文章强调：MCP/tool schemas 如果无差别进入上下文，会在每次请求中反复消耗 token。CFDW 的对应原则是：
...
GitHub 的另一个重要经验是：如果某些数据总是需要，就应该用确定性 CLI 或预处理脚本先拿到，而不是让 agent 在对话中一步步拿。CFDW 对应策略：
...
单看 cache hit 容易漏掉循环问题，因为循环中的请求可能仍然命中缓存。CFDW release audit 和 dashboard 应同时关注：
...
GitHub 文章的经验可以延伸到 CFDW 的跨阶段数据传输：上下游 agent 之间不要传大段自由文本，而应该传稳定结构。
...
这次更新后，CFDW 的 token efficiency 体系从“缓存命中率可见”升级为：
